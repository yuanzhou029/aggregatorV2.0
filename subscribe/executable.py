...
def which_bin() -> tuple[str, str]:
    ...
    if cpu_arch not in ["amd", "arm"]:
        logger.error(
            f"subconverter does not support current cpu architecture: {cpu_arch}")
        sys.exit(1)

    operating_system = platform.system()
    if operating_system not in ["Windows", "Linux", "Darwin"]:
        logger.error(
            f"subconverter does not support current operating system: {operating_system}")
        sys.exit(1)

    if operating_system == "Windows" and cpu_arch != "amd":
        logger.error(
            f"the windows version of subconverter only supports amd64 architecture, current: {cpu_arch}")
        sys.exit(1)
