    ...
    for r in results:
        for k, v in r.items():
            ...
            if k.startswith(SINGLE_LINK_FLAG):
                ...
                if newproxies and isinstance(newproxies, list):
...
def batch_crawl(conf: dict, num_threads: int = 50,
    ...
    if not conf or not conf.get("enable", True):
        ...
        if mode == 1:
            logger.warning(
                f"exit process because mode=1 and crawling task is disabled")
            sys.exit(0)
    ...
    try:
        ...
        if should_persist:
            ...
            try:
                ...
                logger.error(
                    "[CrawlError] load old subscriptions from remote error")
                pass
        ...
        if allow_single_link():
            ...
            logger.info(
                f"[CrawlInfo] crawl finished, found {
                    len(proxies)} proxy links")
            ...
            if len(proxies) > 0:
                try:
                    ...
                    success = pushtool.push_to(
                        content=content, config=linkspushconf, group="proxies")
                    if success:
        ...
        logger.info(
            f"[CrawlInfo] crawl finished, found {
                len(datasets)} subscriptions")

        if mode != 2 and should_persist and peristedtasks:
            survivors = {
                k: v for k,
                v in peristedtasks.items() if not v.pop(
                    "nocache",
                    False)}

            total, rest = len(peristedtasks), len(survivors)
            logger.info(
                f"[CrawlInfo] to be saved subscriptions filtering completed, total: {total}, discard: {
...
def generate_telegram_task(
        channel: str,
        config: dict,
        pages: int,
        limits: int) -> list[list]:
    include = config.get("include", "")
    exclude = config.get("exclude", "").strip()
    pts = config.get("push_to", [])
    params = config.get("config", {})
    if pages <= 1:
        return [[f"https://t.me/s/{channel}", pts,
                 include, exclude, limits, params]]
    ...
    return [[f"https://t.me/s/{channel}?before={x}",
             pts,
             include,
             exclude,
             limits,
             params] for x in arrays[:pages]]
...
def crawl_telegram(users: dict, pages: int = 1, limits: int = 3) -> dict:
    ...
    logger.info(
        f"[TelegramCrawl] finished crawl from Telegram, found {
            len(subscribes)} subscriptions, cost: {cost}")
    return subscribes
...
def crawl_single_repo(
    ...
    if not username or not repo:
        logger.error(
            f"cannot crawl from github, username: {username}\trepo: {repo}")
        return {}
    ...
    url = f"https://api.github.com/repos/{
        username.strip()}/{
        repo.strip()}/commits?per_page={limits}"
...
def crawl_github_repo(repos: dict) -> list[dict]:
    ...
    cost = "{:.2f}s".format(time.time() - starttime)
    logger.info(
        f"[RepoCrawl] finished crawl from Repositorie, found {
            len(subscribes)} subscriptions, cost: {cost}")
    return subscribes
...
def crawl_google(
    ...
    if notinurl and isinstance(notinurl, list):
    ...
    cost = "{:.2f}s".format(time.time() - starttime)
    logger.info(
        f"[GoogleCrawl] finished crawl from Google, found {
            len(collections)} subscriptions, cost: {cost}")
    return collections
...
def crawl_yandex(
    ...
    for page in range(0, pages):
        ...
        groups = re.findall(
            r"<li class=\"serp-item\s+serp-item_card\s?\".*?>([\s\S]*?)</li>",
            content)
        if not groups:
            logger.error(
    ...
    cost = "{:.2f}s".format(time.time() - starttime)
    logger.info(
        f"[YandexCrawl] finished crawl from Yandex, found {
            len(collections)} subscriptions, cost: {cost}")
    return collections
...
def search_github_issues_byapi(
        peer_page: int = 50,
        page: int = 1) -> list[str]:
    ...
    try:
        ...
        logger.error(
            "[GithubIssuesCrawl] occur error when search issues from github")
        traceback.print_exc()
        return []
...
def intercept(text: str, excludes: list = []) -> bool:
    ...
    for regex in excludes:
        try:
            ...
            logger.error(
                f"[GithubRepoIntercept] invalid regex pattern: {regex}")
    return False
...
def crawl_github(
    ...
    if utils.isblank(token):
        ...
        results = utils.multi_thread_run(
            func=search_github_code_byapi, tasks=params)
        items = list(set(itertools.chain.from_iterable(results)))
    ...
    cost = "{:.2f}s".format(time.time() - starttime)
    logger.info(
        f"[GithubCrawl] finished crawl from Github through {method}, found {
            len(subscribes)} subscriptions need check, cost: {cost}")
...
def crawl_pages(
    ...
    for k, v in pages.items():
        ...
        final_headers = deepcopy(headers) if headers and isinstance(
            headers, dict) else utils.DEFAULT_HTTP_HEADERS
        ...
        params.append([k, push_to, include, exclude, config,
                      final_headers, origin, nocache])

    subscribes = multi_thread_crawl(func=crawl_single_page, params=params)
    if not silent:
        cost = "{:.2f}s".format(time.time() - starttime)
        logger.info(
            f"[PageCrawl] finished crawl from Page, found {
                len(subscribes)} subscriptions, cost: {cost}")
...
def username_to_id(username: str, headers: dict) -> str:
    ...
    try:
        ...
        logger.error(
            f"[TwitterCrawl] cannot query uid by username=[{username}]")
        return ""
...
def crawl_twitter(tasks: dict) -> dict:
    ...
    if not guest_token:
        logger.error(
            f"[TwitterCrawl] cannot extract X-Guest-Token from twitter")
        return {}
    ...
    for k, v in tasks.items():
        if utils.isblank(k) or not v or not isinstance(v, dict):
            continue
        candidates[k] = v
    ...
    cost = "{:.2f}s".format(time.time() - starttime)
    logger.info(
        f"[TwitterCrawl] finished crawl from Twitter, found {
            len(subscribes)} subscriptions, cost: {cost}")
...
def extract_subscribes(
    ...
    try:
        ...
        for sub in subscribes:
            ...
            # subconverter url
            if "url=" in sub:
                qs, items = urllib.parse.urlparse(
                    sub.replace("&amp;", "&")).query, []
                ...
                for url in urls:
                    if not utils.isurl(url):
                        if allow_single_link():
                            proxies.extend([x for x in url.split(
                                "|") if re.match(protocal_regex, x, flags=re.I)])
                        continue

                    items.extend([x for x in url.split(
...
def validate(
    ...
    reachable, expired = check_status(
        url=url, retry=2, remain=5, spare_time=12, tolerance=72, connectable=connectable)
    if reachable:
...
def remark(source: dict, defeat: int = 0, discovered: bool = True) -> None:
    if not source or not isinstance(
            source,
            type(defeat)) != int or defeat < 0 or not isinstance(
            discovered,
            bool):
        return
...
def check_status(
    ...
    try:
        ...
        # in order to avoid the request to the speed test site causing constant
        # data downloads, limit the maximum read to 15MB
        ...
        try:
            proxies = yaml.load(
                content, Loader=yaml.SafeLoader).get(
                "proxies", [])
            ...
            proxies = yaml.load(
                content, Loader=yaml.FullLoader).get(
                "proxies", [])
        except BaseException:
            if all(airport.AirPort.check_protocol(x)
...
def is_expired(header: str, remain: float = 0, spare_time: float = 0,
               tolerance: float = 0) -> tuple[bool, bool]:
    ...
    try:
        ...
        expired = False if flag else (
            expire is not None and (
                expire +
                tolerance *
                3600) <= time.time())
        return flag, expired
...
def naming_task(url) -> str:
    ...
    return prefix + "".join(random.sample(string.digits +
                            string.ascii_lowercase, random.randint(3, 5)))
...
def collect_airport(
    ...
    def crawl_ccbh() -> dict:
        ...
        try:
            ...
            if not groups:
                logger.warning(
                    f"[AirPortCollector] cannot found any domains from [{url}]")
                return {}
            ...
            latest = utils.multi_thread_run(
                func=get_redirect_url, tasks=urls, num_threads=num_thread)

            for i, x in enumerate(urls):
                domain = utils.extract_domain(
                    url=latest[i], include_protocal=True)
                if not domain:
                    continue
            ...
            logger.error(
                f"[AirPortCollector] occur error when crawl from [{url}], message: \n{
                    traceback.format_exc()}")

        logger.info(
            f"[AirPortCollector] finished crawl from [{url}], found {
                len(result)} domains")
        return result
    ...
    def crawl_ygpy() -> dict:
        def get_links(
                url: str,
                prefix: str,
                regex: str = r'href="(/vpn/\d{4}/\d{2}.html)"') -> list[str]:
            content = utils.http_get(url=url)

            groups = re.findall(regex, content, flags=re.I)
            if not groups:
                logger.warning(
                    f"[AirPortCollector] cannot fetch article from url: {url}")
                return []

            prefix = utils.trim(prefix)
            return list(set([urllib.parse.urljoin(prefix, x)
                        for x in groups if x]))

        base = "https://ygpy.net"
        links = get_links(
            url=f"{base}/vpn/free",
            prefix=base,
            regex=r"/assets/chunks/free.data\.[^\r\n\s]+\.js")
        if not links:
            logger.warning(
                f"[AirPortCollector] cannot get article from url: {base}")
            return {}

        articles = get_links(
            url=links[0],
            prefix="",
            regex=r'"(https://ygpy.net/vpn/free/\d{4}/\d{2}/[^\r\n\s">]+\.html)"')
        if not articles:
            logger.warning(
                f"[AirPortCollector] cannot get articles from url: {
                    links[0]}")
            return {}
        ...
        tasks = [[x, separator, address_regex, coupon_regex]
                 for x in sorted(articles)]
        ...
        # Extract javascript link from peer page and then parse airport urls
        # and coupons
        ...
        try:
            ...
            if convert:
                links = utils.multi_thread_run(
                    func=get_redirect_url, tasks=tasks, num_threads=num_thread)
            else:
                links = tasks
            ...
            result = {
                utils.extract_domain(
                    url=x,
                    include_protocal=True): "" for x in links if x}
            logger.info(
                f"[AirPortCollector] finished crawl from [{url}], found {
                    len(result)} domains")
    ...
    def run_crawl(
        ...
        try:
            ...
            for group in groups:
                ...
                words = re.findall(
                    utils.trim(address_regex), group, flags=re.M)
                address = words[0] if words else ""
                if not address:
                    continue
                ...
                domain = utils.extract_domain(
                    url=address, include_protocal=True)
                result[domain] = coupon
        except BaseException:
            logger.error(
                f"[AirPortCollector] occur error when crawl from [{url}], message: \n{
    ...
    def extract_backend_url(domain: str, retry: int = 2) -> str:
        ...
        def request_once(suffix: str) -> tuple[bool, str]:
            ...
            while count < retry:
                ...
                try:
                    request = urllib.request.Request(
                        url=url, headers=utils.DEFAULT_HTTP_HEADERS, method="GET")
                    response = urllib.request.urlopen(
                        request, timeout=6, context=utils.CTX)

                    word = "" if not suffix else (
                        suffix if suffix.startswith("/") else "/" + suffix)
                    if word and not utils.trim(
                            response.geturl()).endswith(word):
                        return True, ""
        ...
        def attempt_env() -> str:
            ...
            if groups := re.findall(
                r"\bhost\b:(?:\s+)?[\"\'](https?://[^\s\r\t]+)[\"\']",
                content,
                    flags=re.I):
                return utils.trim(groups[0])

            groups = re.findall(
                r"window.routerBase(?:\s+)?=(?:\s+)?['\"](https?://.*)['\"]",
                content,
                flags=re.I)
    ...
    candidates = {} if not domains else {
        utils.extract_domain(
            x, True): "" for x in domains}
    ...
    # save to file cause they often contain coupons and require common emails
    # to use
    save_candidates(
    ...
    logger.info(
        f"[AirPortCollector] fetched {
            len(domains)} airport, start extracting real routing addresses")
    ...
    records = {
        sites[i]: candidates.get(
            domains[i],
            "") for i in range(
            len(sites)) if sites[i]}

    # check website availability
    logger.info(
        f"[AirPortCollector] extract real base url finished, start to check it now")
    ...
    logger.info(
        f"[AirPortCollector] finished collect airport, availables: {
            len(availables)}")
    return availables
...
def validate_domain(url: str, rigid: bool = True,
                    chuck: bool = False) -> tuple[bool, str]:
    try:
        if not url:
            return False, ""
        ...
        flag = rr.invite or (
            chuck and rr.recaptcha) or (
            rigid and rr.whitelist and rr.verify)
...
def call(
    ...
    try:
        ...
        if subscribes and isinstance(subscribes, list):
...
def execute_script(script: str, params: dict = {}) -> list[dict]:
    try:
        ...
        if not isinstance(subscribes, list):
            logger.error(
                f"[ScriptError] return value error, need a list, but got a {
                    type(subscribes)}")
            return []
        ...
        subscribes = [
            s for s in subscribes if isinstance(
                s, dict) and s.get(
                "push_to", [])]
        return subscribes
    except BaseException:
